...
/* cdr_delay use */
static struct seninf_core *cdr_core;
...
static void dbg_deinit_chmux(struct seninf_ctx *ctx)
{
	...
	if (ctx->dbg_chmux_param) {
		if (ctx->dbg_chmux_param->settings)
...
static int seninf_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable && (cdr_delay != -1)) {
		dev_info(ctx->dev, "[%s] cdr_delay: %#x(%d)\n", __func__, cdr_delay, cdr_delay);

		if (mtk_cam_seninf_en_cdr_delay() < 0)
			dev_err(ctx->dev, "[%s] mtk_cam_seninf_en_cdr_delay failed\n", __func__);

		if (mtk_cam_seninf_s_cdr_delay(cdr_delay) < 0)
			dev_err(ctx->dev, "[%s] mtk_cam_seninf_s_cdr_delay failed\n", __func__);
...
int mtk_cam_seninf_en_cdr_delay(void)
{
	...
	mutex_lock(&cdr_core->mutex);
	list_for_each_entry(ctx, &cdr_core->list, list) {
		if (ctx->streaming) {
			dev_info(ctx->dev, "%s: sensor_idx %d port %d\n",
						__func__, get_sensor_idx(ctx), ctx->port);

			eye_scan_log = kzalloc(DEBUG_OPS_SHOW_LOG_SIZE + 1, GFP_KERNEL);
			if (eye_scan_log != NULL)
				g_seninf_ops->_eye_scan(ctx, EYE_SCAN_KEYS_CDR_DELAY_DPHY_EN, 1,
	...
	cdr_core->cdr_delay = 0;
	return 0;
}
EXPORT_SYMBOL(mtk_cam_seninf_en_cdr_delay);
...
int mtk_cam_seninf_g_cdr_delay(void)
{
	if (!cdr_core) {
		pr_info("%s: seninf core is NULL\n", __func__);
		return -ENODEV;
	...
	pr_info("%s: cdr_delay 0x%x\n",	__func__, cdr_core->cdr_delay);

	return cdr_core->cdr_delay;
}
EXPORT_SYMBOL(mtk_cam_seninf_g_cdr_delay);
