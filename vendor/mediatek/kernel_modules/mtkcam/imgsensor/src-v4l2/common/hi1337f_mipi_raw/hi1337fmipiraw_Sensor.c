 ...
 *============================================================================
 ****************************************************************************/
#include "hi1337fmipiraw_Sensor.h"
static u16 get_gain2reg(u32 gain);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int hi1337f_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hi1337f_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void hi1337f_set_streaming_control(struct subdrv_ctx *ctx, bool enable);
static int hi1337f_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hi1337f_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int hi1337f_open(struct subdrv_ctx *ctx);
static int hi1337f_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static u16 hi1337f_dgain2reg(struct subdrv_ctx *ctx, u32 dgain);

/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_CHECK_SENSOR_ID, hi1337f_check_sensor_id},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, hi1337f_set_streaming_resume},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, hi1337f_set_streaming_suspned},
	{SENSOR_FEATURE_SET_MULTI_DIG_GAIN, hi1337f_set_multi_dig_gain},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	// frame_desc_prev
	{
		...
		.mode_setting_table = hi1337f_preview_setting,
		.mode_setting_len = ARRAY_SIZE(hi1337f_preview_setting),
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 3,
	.ana_gain_step = 1,
	.ana_gain_table = hi1337f_ana_gain_table,
	.ana_gain_table_size = sizeof(hi1337f_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = hi1337f_init_setting,
	.init_setting_len = ARRAY_SIZE(hi1337f_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.get_id = hi1337f_get_imgsensor_id,
	.init_ctx = init_ctx,
	.open = hi1337f_open,
...
const struct subdrv_entry hi1337f_mipi_raw_entry = {
	.name = "hi1337f_mipi_raw",
	.id = HI1337F_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int hi1337f_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	for (i = 0; i < exp_cnt; i++) {
		...
		gains[i] = hi1337f_dgain2reg(ctx, gains[i]);
	}
...
static int hi1337f_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	hi1337f_set_streaming_control(ctx, TRUE);
	return 0;
}
static int hi1337f_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	hi1337f_set_streaming_control(ctx, FALSE);
	return 0;
}

static int hi1337f_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return hi1337f_get_imgsensor_id(ctx, (u32 *)para);
}
...
static int hi1337f_open(struct subdrv_ctx *ctx)
{
	...
	/* get sensor id */
	if (hi1337f_get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
		return ERROR_SENSOR_CONNECT_FAIL;
