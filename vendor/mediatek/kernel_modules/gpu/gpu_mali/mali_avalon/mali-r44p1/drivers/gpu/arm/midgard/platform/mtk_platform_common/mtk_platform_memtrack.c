...
static void memtrack_gpu_memory_detail(void)
{
	struct kbase_device *kbdev = (struct kbase_device *)mtk_common_get_kbdev();
	struct kbase_context *kctx;
	...
	pr_info("GpuDetail: /* Format: kctx | pages | pid  */\n");
	list_for_each_entry(kctx, &kbdev->kctx_list, kctx_list_link) {
		/* print the memory usage and cap for each kctx */
		pr_info("%s-0x%p %10u %10u\n",
		           "kctx",
		           kctx,
		           atomic_read(&(kctx->used_pages)),
		           kctx->tgid);
	}
	pr_info("GpuDetail: /* End */\n");
}
