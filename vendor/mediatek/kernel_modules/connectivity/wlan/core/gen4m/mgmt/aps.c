	...
	uint8_t ucBssIndex)
{
	...
	if ((prAisBssInfo->eConnectionState == MEDIA_STATE_CONNECTED ||
		...
		if (ais->ucAisIndex != AIS_DEFAULT_INDEX) {
			...
			if (mainBssDesc &&
			    IS_AIS_CONN_BSSDESC(mainAis, mainBssDesc)) {
				DBGLOG(APS, INFO,
	...
	if (eRoamReason == ROAMING_REASON_BTM) {
		...
		return scanWtcCheckBssDesc(prAdapter, ucBssIndex, prBssDesc);
#endif
	}
#endif
	...
	struct AP_COLLECTION *ap, uint8_t bidx)
{
	...
	if (link_num > 1) {
		total_score =
			total_score * (ad->rWifiVar.cRCMloTpPref + 100) / 100;
		total_tput =
			total_tput * (ad->rWifiVar.cRCMloTpPref + 100) / 100;
	}
#endif
