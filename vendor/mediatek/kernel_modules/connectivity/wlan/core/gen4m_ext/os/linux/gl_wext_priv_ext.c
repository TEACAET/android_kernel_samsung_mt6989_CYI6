			...
			uint32_t *pu4SetInfoLen)
{
	...
	/* STA only */
	uint8_t ucBssIndex;
	uint32_t u4TxRate; //unit: 100Kbps
	...
	u4TxRate = prAdapter->rLinkQuality.rLq[ucBssIndex].u2TxLinkSpeed / 1000;
	...
	prLlsRateInfo = &(
		...
		ucBssIndex]);
#else /* CFG_SUPPORT_STATS_ONE_CMD */
	prLlsRateInfo = &(prAdapter->arTxRateInfo[ucBssIndex]);
#endif /* CFG_SUPPORT_STATS_ONE_CMD */
