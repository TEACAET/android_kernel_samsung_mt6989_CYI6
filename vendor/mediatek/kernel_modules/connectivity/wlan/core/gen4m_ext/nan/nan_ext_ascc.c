...
static const char * const pcCustomFawTag = "Ext";
...
static u_int8_t NAN_BAND_IN_SAME_TIMELINE_BITMAP(enum ENUM_BAND eBand1Bitmap,
						 enum ENUM_BAND eBand2)
{
	if (eBand1Bitmap & BIT(eBand2))
		return TRUE;

#if (CFG_SUPPORT_NAN_6G == 1)
	if ((eBand1Bitmap & BIT(BAND_5G)) && eBand2 == BAND_6G)
		return TRUE;

	if ((eBand1Bitmap & BIT(BAND_6G)) && eBand2 == BAND_5G)
		return TRUE;
#endif
...
{
	return bitmap & NAN_SLOT_INDEX(slot);
}
#endif
...
static u_int8_t isNanFcScheduleEntrySet(uint32_t schedule_category_bitmap)
{
	return !!(schedule_category_bitmap & FC_ENTRY_BITMAP);
}
...
{
	enum ENUM_BAND eAisBandBitmap = 0; /* bitmap of 1-index */
	...
	for (eBand = BAND_2G4; eBand < BAND_NUM; eBand++) {
		if (nanSchedGetConnChnlUsage(prAdapter, NETWORK_TYPE_AIS, eBand,
	...
	if (nanGetTimelineNum() == 1 ||
	    NAN_BAND_IN_SAME_TIMELINE_BITMAP(eAisBandBitmap, eCustBand)) {
			...
			struct IE_NAN_ASCC_EVENT *prResponse)
{
	...
	for (i = 0; i < ARRAY_SIZE(nanExtSchedEntry); i++) {
		...
		if (isAisScheduleEntry(i)) {
			...
			eBand = b + 1; /* _ENUM_BAND_INDEX_T -> ENUM_BAND */

			nanSchedGetConnChnlUsage(prAdapter, NETWORK_TYPE_AIS,
						 eBand, &rChnlInfo,
						 &u4SlotBitmap, &ucPhyTypeSet);
			if (rChnlInfo.u4PrimaryChnl == 0) {
				DBGLOG(NAN, WARN,
				       "Skip, not same band AIS r.chnl=%u, i=%u",
				       rChnlInfo.u4PrimaryChnl, i);
				continue;
			}
				  ...
				  struct IE_NAN_ASCC_CMD *cmd)
{
	...
	if (prNDL) { /* Set/clear FR flags accordingly */
		if (cmd->usage == NAN_SCHEDULE_USAGE_ASCC_PS_FR ||
			...
			nanExtSetFr(prNDL, cmd->ucRequestId, kalGetBootTime());
		} else if (cmd->usage != NAN_SCHEDULE_USAGE_CHECK_SCHEDULE) {
			/* Set u4SetFastRecovery to 0 to clear FR */
			if (nanExtGetFr(prNDL, &ucRequestId, NULL))
				nanExtSetFr(prNDL, ucRequestId, 0);
		}
	}
}
...
void nanExtReconfigureCustFaw(struct ADAPTER *prAdapter)
{
	enum _ENUM_BAND_INDEX_T band;
	enum NAN_SCHEDULE_CATEGORY entry_idx;
	uint32_t custom_schedule_entry =
	...
	nanSchedNegoCustFawRemoveEntry(prAdapter, pcCustomFawTag);
	...
	for (band = NAN_EXT_BAND_2G4, entry_idx = CATEGORY_FC_2G;
		...
		nanSchedNegoCustFawAddEntry(prAdapter,
					    ...
					    .pcTag = pcCustomFawTag,
					    .ucOpChannel = faw->ucOpChannel,
					    .eBand = bandTable[band],
					    .u4Bitmap = faw->u4Bitmap});
	}
	...
	for (band = NAN_EXT_BAND_2G4, entry_idx = CATEGORY_USD_2G;
		...
		nanSchedNegoCustFawAddEntry(prAdapter,
					    &(struct _NAN_CUST_FAW_ENTRY){
					    .pcTag = pcCustomFawTag,
					    .ucOpChannel = usd->ucOpChannel,
					    .eBand = bandTable[band],
					    .u4Bitmap = usd->u4Bitmap});
	}
	...
	for (band = NAN_EXT_BAND_2G4, entry_idx = CATEGORY_SD_2G;
		...
		nanSchedNegoCustFawAddEntry(prAdapter,
					    &(struct _NAN_CUST_FAW_ENTRY){
					    .pcTag = pcCustomFawTag,
					    .ucOpChannel = sd->ucOpChannel,
					    .eBand = bandTable[band],
					    .u4Bitmap = sd->u4Bitmap});
	}
...
static void nanAsccSetFastConnect(struct ADAPTER *prAdapter, u_int8_t fgEnable)
{
	...
	if (!fgEnable &&
	    !isNanFcScheduleEntrySet(gAsccRecord.custom_schedule_entry))
		return;
	...
	*cmd = (struct IE_NAN_ASCC_CMD) {
	...
	if (prAdapter->rWifiVar.fgNanAutoFC == 2) { /* 2G + 5G */
		...
		*prFc2G = (struct IE_NAN_ASCC_FAW) {
			...
			.u4Bitmap = u4Bitmap,
			.ucOpChannel = g_r2gDwChnl.u4PrimaryChnl,
		};
	...
	*prFc5G = (struct IE_NAN_ASCC_FAW) {
		...
		.operation = NAN_SCHEDULE_ENTRY_ADD,
		.u4Bitmap = u4Bitmap,
		.ucOpChannel = g_r5gDwChnl.u4PrimaryChnl,
	};
...
{
	nanAsccSetFastConnect(prAdapter, fgEnable);
}
...
uint32_t nanGetFcSlots(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(NAN, TEMP, "FC slots: %02x-%02x-%02x-%02x\n",
			       ...
			       size_t szSlotIdx)
{
	...
	if (NAN_IS_5G_TIMELINE(prAdapter, szTimelineIdx)) {
		band = NAN_EXT_BAND_5G;
		entry_idx = CATEGORY_FC_5G;
		szLimit = ARRAY_SIZE(gAsccRecord.arFcEntry);
	} else {
		band = NAN_EXT_BAND_2G4;
		entry_idx = CATEGORY_FC_2G;
		szLimit = NAN_EXT_BAND_5G;
	}
...
#define NAN_SLOT_IS_M4_CH_SWITCH(_szSlotIdx)\
	...
	if (prAdapter->rWifiVar.ucNanEhtCHSwitchMode == 4 &&
		...
		NAN_DW_DBGLOG(NAN, INFO, fgPrintLog, szSlotIdx,
			      "Tidx(%zu) CH switch slot(%zu): Force empty for M4 CH switch\n",
			      szTimeLineIdx, szSlotIdx);
		return TRUE;
	}

#undef NAN_SLOT_MASK_TYPE_M2_CH_SWITCH
#undef NAN_SLOT_MASK_TYPE_M4_CH_SWITCH
#undef NAN_SLOT_IS_M2_CH_SWITCH
...
uint32_t nanAsccBackToNormal(struct ADAPTER *prAdapter)
{
	...
	if (*pu2Length != sizeof(struct IE_NAN_ASCC_CMD) - ASCC_CMD_HDR_LEN)
		nanSchedUpdateCustomCommands(prAdapter, buf,
					     ASCC_CMD_SIZE(buf));
